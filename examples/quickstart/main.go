// Command quickstart runs one classification through the systemone client.
//
...
func run() error {
	...
	started := time.Now()
	result, err := client.Evaluate(ctx, state, systemone.Questions{
...
func describe(err error) error {
	...
	case errors.Is(err, systemone.ErrConfig):
		return fmt.Errorf("the configuration is unusable: %w", err)
	case errors.Is(err, systemone.ErrAborted), errors.Is(err, systemone.ErrTimeout):
		return fmt.Errorf("gave up before the host answered: %w", err)
	case errors.Is(err, systemone.ErrConnection):
