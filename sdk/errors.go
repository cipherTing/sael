...
var (
	...
	ErrConfig = errors.New("sdk: invalid configuration")
)
...
func (e *APIError) Error() string {
	if e.Message == "" {
		return fmt.Sprintf("sdk: api error: HTTP %d", e.StatusCode)
	}
	return fmt.Sprintf("sdk: api error: HTTP %d: %s", e.StatusCode, e.Message)
}
...
func (e *ConnectionError) Error() string { return "sdk: connection failed: " + e.Err.Error() }
...
func (e *TimeoutError) Error() string {
	if e.Timeout > 0 {
		return fmt.Sprintf("sdk: request timed out after %s: %v", e.Timeout, e.Err)
	}
	return "sdk: request timed out: " + e.Err.Error()
}
...
func (e *AbortError) Error() string { return "sdk: request aborted by caller: " + e.Err.Error() }
