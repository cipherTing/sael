...
func (t *transport) do(ctx context.Context, req *httpRequest) (*httpResponse, error) {
	if req == nil {
		return nil, errors.New("sdk: transport: nil request")
	}
	...
	for attempt := 1; ; attempt++ {
		...
		if attempt > 1 {
			...
			if t.log.Enabled(ctx, slog.LevelDebug) {
				t.log.DebugContext(ctx, "sdk: backing off before retry",
		...
		if err == nil {
			...
			if t.log.Enabled(ctx, slog.LevelWarn) {
				t.log.WarnContext(ctx, "sdk: retrying after HTTP status",
		...
		if t.log.Enabled(ctx, slog.LevelWarn) {
			t.log.WarnContext(ctx, "sdk: retrying after transport error",
...
func (t *transport) attempt(ctx context.Context, req *httpRequest, url string, attempt int) (*httpResponse, error) {
	...
	if t.log.Enabled(ctx, slog.LevelDebug) {
		...
		t.log.DebugContext(ctx, "sdk: sending request",
			"method", req.method,
			"url", url,
			"attempt", attempt,
	...
	if t.log.Enabled(ctx, slog.LevelDebug) {
		t.log.DebugContext(ctx, "sdk: received response",
			"method", req.method,
			"url", url,
			"attempt", attempt,
	...
	if policy.RespectRetryAfter {
		if after, ok := parseRetryAfter(lastHeader); ok {
			...
			if t.log.Enabled(context.Background(), slog.LevelDebug) {
				t.log.Debug("sdk: ignoring Retry-After beyond MaxRetryAfter",
...
func (t *transport) logAttemptFailure(ctx context.Context, req *httpRequest, attempt int, elapsed time.Duration, err error) {
	...
	t.log.WarnContext(ctx, "sdk: request attempt failed",
		"method", req.method,
		"attempt", attempt,
		"duration", elapsed.String(),
		"error", err)
