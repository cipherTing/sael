...
//   cd sdk && go test -run Integration -count=1 -v ./...
//
// From the repository root, `make test-integration` does the same thing and also
// checks that a key is present before starting, so a missing one reads as a
// missing one rather than as a suite that quietly skipped everything.
//
