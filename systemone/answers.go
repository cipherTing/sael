...
func decodeResult(b []byte) (*Result, error) {
	...
	if err := json.Unmarshal(b, &wire); err != nil {
		return nil, fmt.Errorf("systemone: invalid response body: %w", err)
	}
	...
	for name, raw := range wire.Answers {
		...
		if err != nil {
			return nil, fmt.Errorf("systemone: answer %q: %w", name, err)
		}
		if answer == nil {
...
func decodeScoreProbabilities(m map[string]json.RawMessage) ([]float64, error) {
	...
	for _, entry := range entries {
		...
		if err := json.Unmarshal(entry.value, &value); err != nil {
			return nil, fmt.Errorf("systemone: score probability %q is not a number: %w", entry.key, err)
		}
