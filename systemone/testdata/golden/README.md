...
```console
$ for f in systemone/testdata/golden/*.json; do python3 -m json.tool "$f" >/dev/null && echo "OK $f"; done
OK   ...                                  # all 12 files

$ grep -r '<the OpenRouter key prefix, "sk-" + "or-v1">' systemone/testdata/
                                          # no output, exit status 1

$ grep -rE 'Bearer [A-Za-z0-9_.-]' systemone/testdata/
                                          # no output, exit status 1
```
