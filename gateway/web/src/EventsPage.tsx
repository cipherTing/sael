  ...
  return <Stack spacing={2.5}>
    ...
    <Paper variant="outlined" sx={{ p: 2, borderRadius: 3 }}>
      <Stack direction={{ xs: 'column', md: 'row' }} spacing={1.5} sx={{ alignItems: { md: 'center' } }}>
        <FormControl size="small" sx={{ minWidth: 135 }}><InputLabel>时间</InputLabel><Select label="时间" value={filters.hours} onChange={e => setFilters({ ...filters, hours: Number(e.target.value) })}><MenuItem value={1}>近 1 小时</MenuItem><MenuItem value={6}>近 6 小时</MenuItem><MenuItem value={12}>近 12 小时</MenuItem><MenuItem value={24}>近 24 小时</MenuItem><MenuItem value={168}>近 7 天</MenuItem><MenuItem value={336}>近 14 天</MenuItem><MenuItem value={720}>近 30 天</MenuItem></Select></FormControl>
    ...
    <Paper variant="outlined" sx={{ borderRadius: 2, overflow: 'hidden' }}>
      {!loading && !events.length ? <Box sx={{ px: 3, py: 4 }}><Typography sx={{ fontWeight: 650 }}>暂无数据</Typography></Box> : <DataGrid rows={events} columns={columns} loading={loading} onRowClick={params => setSelected(params.row)} disableRowSelectionOnClick initialState={{ pagination: { paginationModel: { pageSize: 10 } } }} pageSizeOptions={[10, 25, 50]} autoHeight sx={{ border: 0, minHeight: 240, '& .MuiDataGrid-row': { cursor: 'pointer' } }} localeText={{ ...zhCN.components.MuiDataGrid.defaultProps.localeText, noRowsLabel: '暂无数据' }} />}
    </Paper>
