...
func TestModerationHasTheExpectedQuestions(t *testing.T) {
	...
	assert.Equal(t, 2, counts["score"],
		"two graded questions, and only two: how explicit the sexual content is, and "+
			"how graphic the violence is. Severity is deliberately not a third, "+
			"because a Noul's 0.5 does not mean \"medium\" and a separate severity "+
			"score would be read as one; see the comments above both in moderation.go")
...
func TestModerationPassesTheClientsOwnValidation(t *testing.T) {
	require.NoError(t, sdk.ValidateQuestions(Moderation()),
...
func TestEveryNoulWritesBothSides(t *testing.T) {
	...
	for _, name := range sortedNames(Moderation()) {
		noul, ok := Moderation()[name].(sdk.NoulQuestion)
		if !ok {
			continue
		}
		nouls++
...
func TestTheScaledQuestionIsAnOrderedRubric(t *testing.T) {
	...
	for _, name := range sortedNames(Moderation()) {
		question, ok := Moderation()[name].(sdk.ScoreQuestion)
		if !ok {
			continue
		}
		scored++
	...
	assert.Equal(t, 2, scored, "the number of Score questions changed")
}

// TestBothGradedQuestionsStartAtTheBenignEnd pins the ordering convention the
...
// caller had already tuned, and on a graded axis that is the difference between
// blocking gore and blocking all fiction.
func TestBothGradedQuestionsStartAtTheBenignEnd(t *testing.T) {
	benign := map[string]string{
		"sexual": "not sexual",
		"gore":   "not violent",
	}

	for name, phrase := range benign {
		question, ok := Moderation()[name].(sdk.ScoreQuestion)
		require.True(t, ok, "%s should be a Score question", name)

		first := strings.ToLower(entryText(question.Criteria[0]))
		assert.Contains(t, first, phrase,
			"level 0 of %s is no longer the benign end of the scale", name)

		for i, level := range question.Criteria[1:] {
			assert.NotContains(t, strings.ToLower(entryText(level)), phrase,
				"level %d of %s claims the benign reading, but level 0 is where it "+
					"belongs", i+1, name)
		}
	}
}
...
// string.
func renderModeration(set sdk.Questions) string {
...
func questionInstruction(q sdk.Question) string {
	switch question := q.(type) {
	case sdk.NoulQuestion:
		return entryText(question.Instructions)
	case sdk.ScoreQuestion:
		return entryText(question.Instructions)
	case sdk.ChoiceQuestion:
		return entryText(question.Instructions)
	default:
		return ""
	}
...
func entryText(entry sdk.Entry) string {
...
func entriesText(entries []sdk.Entry) []string {
...
func sortedNames(set sdk.Questions) []string {
