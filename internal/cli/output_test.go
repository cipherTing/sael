...
func TestRankedHazardsOrdersByStrengthThenName(t *testing.T) {
	result := newResult(map[string]systemone.Answer{
		"delta": systemone.NoulAnswer{Noul: 0.10},
		"alpha": systemone.NoulAnswer{Noul: 0.90},
		"gamma": systemone.NoulAnswer{Noul: 0.50},
		...
		"bravo": systemone.NoulAnswer{Noul: 0.50},
		"echo":  systemone.NoulAnswer{Noul: 0.00},
	})
...
func TestRankedHazardsIgnoresAnswersThatAreNotYesOrNo(t *testing.T) {
	result := newResult(map[string]systemone.Answer{
		"a_noul":  systemone.NoulAnswer{Noul: 0.80},
		"a_score": systemone.ScoreAnswer{Score: 3, Confidence: 1, Legend: []string{"a", "b", "c", "d"}},
		"a_choice": systemone.ChoiceAnswer{
			Choice:        "yes",
			Confidence:    0.9,
...
func TestScaledAnswersAreSortedByName(t *testing.T) {
	result := newResult(map[string]systemone.Answer{
		"zeta":  systemone.ScoreAnswer{Score: 1, Legend: []string{"a", "b"}},
		"alpha": systemone.ScoreAnswer{Score: 1, Legend: []string{"a", "b"}},
		"noul":  systemone.NoulAnswer{Noul: 0.5},
	})
...
`, buf.String(),
		"the terminal view is what a person reads to decide whether anything fired, "+
			"so its layout is worth pinning: the bars length out the probability, and "+
			"a bare 2.50 is only meaningful with the level name beside it")
}
...
func TestWriteReportDrawsABarProportionalToTheValue(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(fmt.Sprintf("%.3f", tc.value), func(t *testing.T) {
			var buf bytes.Buffer
			require.NoError(t, writeReport(&buf, newResult(map[string]systemone.Answer{
				"x": systemone.NoulAnswer{Noul: tc.value},
			}), false))
...
// total no level reached, so "1.00 / 3" described a position on a scale that only
// had two levels.
func TestWriteReportReadsTheRubricSizeFromTheAnswer(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			var buf bytes.Buffer
			require.NoError(t, writeReport(&buf, newResult(map[string]systemone.Answer{
				"s": systemone.ScoreAnswer{Score: tc.score, Confidence: 1, Legend: tc.legend},
			}), false))
...
func TestWriteReportPutsAScaleOnlyReportOnOneLine(t *testing.T) {
	var buf bytes.Buffer
	require.NoError(t, writeReport(&buf, newResult(map[string]systemone.Answer{
		"sexual": systemone.ScoreAnswer{Score: 1, Confidence: 1, Legend: []string{"none", "mild"}},
	}), false))
...
func TestWriteReportLinesUpNamesOfDifferentLengths(t *testing.T) {
	var buf bytes.Buffer
	require.NoError(t, writeReport(&buf, newResult(map[string]systemone.Answer{
		"a":                  systemone.NoulAnswer{Noul: 0.5},
		"a_much_longer_name": systemone.NoulAnswer{Noul: 0.4},
		"mid_length":         systemone.NoulAnswer{Noul: 0.3},
	}), false))
...
func TestWriteReportReportsAWriteFailure(t *testing.T) {
	...
	t.Run("while drawing a scale", func(t *testing.T) {
		...
		err := writeReport(failingWriter{}, newResult(map[string]systemone.Answer{
			"s": systemone.ScoreAnswer{Score: 1, Confidence: 1, Legend: []string{"a", "b"}},
		}), false)
		require.Error(t, err)
		assert.Contains(t, err.Error(), "broken pipe")
	})
...
func TestLevelTextNamesTheNearestLevel(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(fmt.Sprintf("%.2f", tc.score), func(t *testing.T) {
			assert.Equal(t, tc.want, levelText(systemone.ScoreAnswer{Score: tc.score, Legend: legend}),
...
func TestLevelTextOnARubricWithNoDescription(t *testing.T) {
	...
	assert.Equal(t, "", levelText(systemone.ScoreAnswer{Score: 2, Legend: nil}))
	assert.Equal(t, "", levelText(systemone.ScoreAnswer{Score: 2, Legend: []string{}}),
...
// produces.
func sampleResult() *systemone.Result {
	return newResult(map[string]systemone.Answer{
		"alpha": systemone.NoulAnswer{Noul: 0.90},
		"beta":  systemone.NoulAnswer{Noul: 0.40},
		"zzz": systemone.ScoreAnswer{
			Score:         2.50,
			Confidence:    0.80,
			Legend:        []string{"none", "mild", "serious", "severe"},
...
func newResult(answers map[string]systemone.Answer) *systemone.Result {
	return &systemone.Result{Model: "jev-test", Provider: "test", ID: "req-1", Answers: answers}
}
